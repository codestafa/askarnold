...
// Child: fetches workouts + pagination UI
function WorkoutsContent({
  ...
  const goTo = (page: number) => {
    router.push(`/workouts?page=${page}`);
  };
    ...
    <>
      <div className="grid grid-cols-1 gap-6">
        {workouts.map((w) => (
          <div
            key={w.id}
            className="border bg-white p-5 rounded-lg shadow hover:shadow-md transition"
          >
            <h5 className="text-lg font-semibold text-gray-900 mb-2">
              Workout #{w.id}
            </h5>
            <p className="text-gray-600 mb-4 line-clamp-3">{w.planText}</p>
...
export default function WorkoutsPage() {
  ...
  // while we haven't determined desktop vs mobile
  if (isDesktop === null) {
    return null;
  }
  ...
  const sections = [
    {
      name: "chat",
      icon: "M3 9.5L12 3l9 6.5v9.5a2 2 0 01-2 2h-4a2 2 0 01-2-2v-4H9v4a2 2 0 01-2 2H3a2 2 0 01-2-2V9.5z",
    },
    {
      name: "workouts",
      icon:
